...
def scheme_eval(expr, env, _=None):
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        map_func = lambda curr_operand: scheme_eval(curr_operand, env)
        eval_operands = rest.map(map_func)
...
def eval_all(expressions, env):
    ...
    while expressions is not nil:
        evaluate, expressions = scheme_eval(expressions.first, env), expressions.rest
    return evaluate
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        lambda_proc_instance = do_lambda_form(Pair(formals, body), env)
        env.define(name, lambda_proc_instance)
        return name
    else:
...
def do_and_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if is_false_primitive(evaluated):
            return False
...
def do_or_form(expressions, env):
    ...
    while expressions is not nil:
        evaluated = scheme_eval(expressions.first, env)
        if is_true_primitive(evaluated):
            return evaluated
        expressions = expressions.rest
    return False
